...
public class type17
{
    ...
    {
        public skill312() : base(312, "챠밍보이스", 40, 100, 17, 0, false)
        {
        }
        public override IEnumerator skill_effect(y_color attacker, y_color defender)
        {
            ...
            GameObject prefab = Resources.Load<GameObject>("Prefab/CharmingVoice");
    ...
    public class skill402 : monoskill
    {
        public skill402() : base(402, "매지컬샤인", 80, 100, 17, 17, false, 6, "광역 상대(1)")
        {
        }
        public override IEnumerator skill_effect(y_color attacker, y_color defender)
        {
            ...
            GameObject prefab = Resources.Load<GameObject>("Prefab/FairyShine");
            GameObject go = UnityEngine.Object.Instantiate(
                prefab,
                attacker.transform.position,
                prefab.transform.rotation
        ...
        public override IEnumerator use_skill(y_color attacker, y_color defender)
        {
            ...
            if (attacker.cc.effect(hit_dice))
            {
                ...
                foreach (y_color unit in allUnits)
                {
                    if (Vector3.Distance(defender.transform.position, unit.transform.position) <= 1f)
                    {
                        if (unit.GetType() != targetType) continue;
